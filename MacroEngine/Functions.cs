...
namespace Funcitons
{
    public static class NormalFunc
    {
        ...
        /// 在程序运行目录Log.log中写日志
        /// </summary>
        /// <param name="level">日志等级，通常有INFO WARN ERROR DEBUG等</param>
        ...
        /// <returns>返回string格式的延迟(ms)，连接失败返回"unconnect"</returns>
        ...
        public static void ShowSystemNotification(string title, string content)
        {
        ...
        /// <returns>颜色</returns>
        public static Color GetPixelColor_Bitmap(Point position)
        {
            // 创建与屏幕大小相同的位图
            using (Bitmap screenBitmap = new Bitmap(
                ...
                height: Screen.PrimaryScreen.Bounds.Height))
            {
                ...
                using (Graphics graphics = Graphics.FromImage(screenBitmap))
                {
                    graphics.CopyFromScreen(
                        ...
                        destinationX: 0,
                        destinationY: 0,
                        blockRegionSize: screenBitmap.Size,
                        copyPixelOperation: CopyPixelOperation.SourceCopy);
                }

                // 返回指定坐标的像素颜色
                return screenBitmap.GetPixel(position.X, position.Y);
            }
        }
    ...
    {
        private readonly List<Action<Graphics>> _drawCommands = new List<Action<Graphics>>();
        private readonly Pen _defaultPen = new Pen(Color.Red, 2);
        private readonly Brush _defaultBrush = new SolidBrush(Color.FromArgb(128, Color.Blue));
        ...
        public Overlay()
        {
            ...
            // 透明设置
            BackColor = Color.Magenta;  // 透明色
            TransparencyKey = Color.Magenta;
            Opacity = 0.99;  // 避免全透明导致无法绘制
        ...
        {
            _drawCommands.Add(g => g.DrawRectangle(pen ?? _defaultPen, rect));
            Invalidate();
        }

        public void DrawFullRectangle(Rectangle rect, Brush brush = null)
        {
            _drawCommands.Add(g => g.FillRectangle(brush ?? _defaultBrush, rect));
            Invalidate();
        }

        public void DrawLine(Point start, Point end, Pen pen = null)
        {
            _drawCommands.Add(g => g.DrawLine(pen ?? _defaultPen, start, end));
            Invalidate();
        }

        public void DrawText(string text, Point position, Font font = null, Brush brush = null)
        {
            _drawCommands.Add(g =>
            {
                using (var f = font ?? new Font("Arial", 12))
                using (var b = brush ?? new SolidBrush(Color.White))
                {
